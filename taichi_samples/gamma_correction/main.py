...
@ti.func
def InvGammaCorrection(color):
    return ti.math.pow(color, 2.2)

@ti.kernel
def GammaCorrectionKernel(img: Img2d, bInvGC: ti.u8):
    h, w = img.shape[0], img.shape[1]
    for i, j in ti.ndrange(h, w):
        c = Vector3(img[i, j] / 255.0)
        if bInvGC > ti.u8(0):
            img[i, j] = (InvGammaCorrection(c) * 255.0).cast(ti.u8)
        else:
